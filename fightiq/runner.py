...
from fightiq.config import EXPORT_DIR, LOG_DIR, UPLOAD_TO_SUPABASE, MAX_EVENTS, MAX_FIGHTERS, MAX_FIGHTS
...
from fightiq.analytics.derived import add_basic_fighter_metrics
...
def main():
    ...
    fighters, fighter_history = scrape_fighters(include_details=True)
    fighters = add_basic_fighter_metrics(fighters)
    write_csv("fighters", fighters)
    write_csv("fighter_history_raw", fighter_history)
    report["fighters"] = len(fighters)
    report["fighter_history_raw"] = len(fighter_history)

    # Optional normalized fighter_stats table
    fighter_stats_cols = [
        "id", "full_name", "slug", "slpm", "sapm", "strike_accuracy",
        "strike_defense", "takedown_avg", "takedown_accuracy",
        "takedown_defense", "submission_avg", "win_rate", "elo_rating"
    ]
